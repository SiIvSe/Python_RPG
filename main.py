...
print(Fore.BLUE + f"Здравствуй {name}! Теперь выбери своё оружие: \n 1 - катана \n  2 - двуручный меч \n 3 - коса \n 4 - кинжал \n копьё")  # обатиться по имени расказать о видах оружия и дать выбор
print("_" * 150)
    ...
    while True:
        ...
        # выбор оружия кинжал__________________________________________________________________________________________________________________________________________________________________
            ...
            return Weapon(name = "Копьё",
                            ...
                            har = f"Копьё - отличный выбор! Его урон = {80}, шанс попасть по врагу крайне низкий, а урон крайне выоский, даёт дополнительно хп {110}, может ударить 4 врага одновремено! к основному. Двуручный меч - это баланс шанса попасть и урона!"
                            )
            break
        ...
        if wst < 0.4:
            ...
            while contr_mob > 0 and hp > 0:
                ...
                if chu <= weapon.chup:
                    if ku <= 0.2 and (weapon.name == "кинжал" or weapon.name == "катана"):  # УЛЬТА
                        print("_" * 70)
                        print(Fore.MAGENTA + "Кажиться вы смогли увернуться и есть шанс использовать магию!")
                        ...
                        if hp_mob <= 0:
                            ...
                            hp_mob = wolfe[0]
                        # hp -= uron_mob
                        print(Fore.CYAN + f"удар, и вы востановили немного хп за счёт магии оружия {hp}!, так как вы смогли увернуться. осталось волков {contr_mob}")
                        time.sleep(0.5)
                        time.sleep(1)
                    elif weapon.shans < ch:
                        # hp -= uron_mob
            ...
            while True:
                ...
                if hp <= 0:
                    ...
                    break
            print("_" * 150)
        elif wst < 0.6 and wst > 0.8 and skilet == False:
            ...
            print(Fore.YELLOW + f"Вы встретили торговца, ваш опыт: {opt}")
...
if hp > 0:
    ...
    if ch >= weapon.chup:
        ...
        print(Fore.CYAN + f"Кажиться вы попали прямо по горлу урон был больше обычного {weapon.uron + dop}. \n Но это не конец!")
        time.sleep(0.5)
    print("_" * 150)
    ...
    if ch >= weapon.chup:
        ...
        hp -= 40
        hp_med -= (weapon.uron + dop)
        print(Fore.CYAN + f"Кажиться что он уже на исдыхании {weapon.uron + dop}. \n Но это не конец!")
        time.sleep(0.5)
    hp = hps
    print("_" * 50, "\n")
    time.sleep(0.5)
    if bear:
        print(Fore.CYAN + "Ему уже конец, неужели это изза его деревня пуста?... \n")
        time.sleep(0.5)
    else:
    ...
    print(Fore.CYAN + "тут раздался взрыв и вышел тот самый ненакомец навел на вас лук и холднокровно сказал. \n У вас два пути ваши оружия не достанут до меня отдайти мне ваши монеты или ...")
    time.sleep(0.5)
    ch = chance()
    print("_" * 150)
    if monet >= 1000:
    ...
    print("_" * 150)
    print(Fore.RED + Style.BRIGHT + "Вы проиграли... От ныне будьте внимательнее прежде чем на что то соглашаться, удачи и до новых встречь ;)")
    time.sleep(0.25)
    print("=" * 50)
    time.sleep(0.25)
    print(f"Имя: {name}")
